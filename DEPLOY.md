...
# 判据 = 名称 ∈ 演示物料清单 + 区县 ∈ 演示区县 + 有效期存在且已过期（三条缺一不可）：
#   - 不用批号（同一套演示数据在不同环境批号不同，会静默漏掉）
#   - 必须限定区县（现场 4 个区县各有同名物料，只有演示区县那几套是演示数据）
#     ⚠ 演示区县是**复数**（SEED_MATERIAL_DISTRICT_CODES = 襄城区 + 樊城区），
#       漏掉任何一个，那个区县的演示物料过期后就永远修不回来，而命令不报错
