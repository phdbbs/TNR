...
def expired_material_error(material, action='使用'):
    ...
    * **不拦「医院签收」** —— 否则已下发的过期物料会卡在「待签收」，
    ...
    ⚠ **「下发」暂不拦**，但理由已变（第三十七轮）。原先的硬理由是
    「捕捉点角色的 `stock_adjustment` 权限里没有 shelter，拦了下发，
    捕捉点的过期物料**没有任何出口**」—— 那个权限缺口**本轮已补上**
    （`views_material.stock_adjustment` 现在含 `shelter`，前端捕捉点门户
    新增「库存异动」tab），所以「没有出口」不再成立。
    但**拦截面本身是行为变更**，会改变现有演示路径，故仍维持不拦，
    留给磊哥决策；判据随时可以打开，出口已经在了。
